...
class SimulatorModel(Configurable):
    ...
    def getTCitem(self, name):
        if self.tcItems.has_key(name):
            ...
            return tcItem
    def getPTCProxy(self,name):
        if self.ptcItems.has_key(name):
            return self.ptcItems[name]
        else:
            ptcItem = PtcItemSimClass(self, name,'Referenced PTC Item Proxy',proxy=True)
            self.ptcItems[name] = ptcItem
            return ptcItem
    def getTCProxy(self,name):
        if self.tcItems.has_key(name):
            return self.tcItems[name]
        else:
            tcItem = TcItemSimClass(self, name,'Referenced TC Item Proxy',proxy=True)
            self.tcItems[name] = tcItem
            return tcItem
    def getTMProxy(self,name):
        if self.tmItems.has_key(name):
            return self.tmItems[name]
        else:
            tmItem = TmItemSimClass(self, name,'Referenced TC Item Proxy',proxy=True)
            self.tmItems[name] = tmItem
            return tmItem
    #===========================================================================    
