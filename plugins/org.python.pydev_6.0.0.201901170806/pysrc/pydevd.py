...
if __name__ == '__main__':
    try:
        ...
        if setup['tmtc-db-server']:
            print 'Using TMTC Server: ' + setup['tmtc-db-server']
        if setup['use-remote-driver']:
            print 'Using SPELL Remote driver'
        else:
            print 'Using emulated SPELL driver'
        #ENDIF
        if setup['use-remote-port']:
            print 'Using Remote Driver with port: ' + str(setup['use-remote-port'])
...
from spelldebugger.spellwrapper import *
...
class PyDB:
    ...
    def __init__(self):
        ...
        self._lock_running_thread_ids = threading.RLock()
    ...
    def do_wait_suspend(self, thread, frame, event, arg, suspend_type="trace"): #@UnusedVariable
        ...
        #print str(CMD_STEP_RETURN)+':' + str(info.pydev_step_cmd) + ':' + repr(info)
        try:
            from spell.lib.registry import REGISTRY  # @UnresolvedImport
            useFilter = REGISTRY['DEBUG'].shouldUseStepFilter()
            del REGISTRY
        except:
            useFilter = SetupHolder.setup['use-step-filter']
            
        if useFilter:
            #traceback.print_stack(frame)
            back_frame = frame
            #print 'init:' + back_frame.f_code.co_filename
            if not info.no_wait:
                while back_frame and not ('UserLib' in back_frame.f_code.co_filename or 'Procedures' in back_frame.f_code.co_filename):
                    #print 'step return:' + back_frame.f_code.co_filename
                    back_frame = back_frame.f_back
                    info.pydev_step_cmd = CMD_STEP_RETURN
                    time.sleep(0.01)
        #print str(CMD_STEP_RETURN)+':' + str(info.pydev_step_cmd) + ':' + repr(info)
                    ...
                    if frame.f_lineno == line:
                        ...
                        try:
                            ...
                            try:
                                ...
                                print >>sys.stderr,'Could not execute Goto!'
    ...
    def run(self, file, globals=None, locals=None, is_module=False, set_trace=True):
        ...
        setupUserLib(os.path.split(file)[-1],globals,locals)
        try:
            REGISTRY['DEBUG'].setNextStep(file,'','')
            REGISTRY['DEBUG'].setNextStep(file,'','')
        except:
            traceback.print_exc()
        if not is_module:
        ...
        try:
            REGISTRY['REMOTE'].Display('Procedure Finished. Closing Debug Server.')
            REGISTRY['REMOTE'].Finish('Procedure Finished. Closing Debug Server.')
        except:
            pass
        try:
            from spelldebugger.spellwrapper import PROC
            if PROC[STEP] != None:
                REGISTRY['DEBUG'].setNextStep(file,PROC[STEP][0],'')
            REGISTRY['DEBUG'].dispose()
        except:
            traceback.print_exc()
...
_set_trace_lock = threading.RLock()
...
def setupUserLib(file,globals,locals):
    #print 'load UserLib'
    import _pydev_bundle.pydev_import_hook as pydev_import_hook
    
    import spelldebugger.spellwrapper as spell1
    for key in spell1.__dict__.keys():
        if key not in globals:
           globals[key] = spell1.__dict__[key]
    globals['PROC']['NAME'] = file[:-3] + '#0'
    globals['__import__'] = pydev_import_hook.import_hook_manager.do_import
...
def main():
    ...
    if fix_app_engine_debug:
        ...
        try:
            print 'Trying to connect to ' + str(host if host else 'localhost') + ' on port ' + str(port)
            debugger.connect(host, port)
        except:
            sys.stderr.write("Could not connect to %s: %s\n" % (host, port))
            traceback.print_exc()
            sys.exit(1)
        print 'Debugger connected to ' + host + ' on port ' + str(port)
